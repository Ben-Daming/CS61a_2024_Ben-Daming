...
class Ant(Insect):
    ...
    def double(self):
        ...
        if not self.is_doubled and self.damage:
            self.damage*=2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while(index<self.lower_bound and not now_place.is_hive):
            index+=1
            now_place=now_place.entrance
        while(index<=self.upper_bound and not now_place.is_hive):
            index+=1
            if now_place.bees:
    ...
    def action(self, gamestate):
        if self.cooldown:
            ...
            if(self.place.bees):
                ...
                self.cooldown=type(self).chew_cooldown
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        place_to_double=self.place.exit
        assert isinstance(place_to_double,Place)
        while(place_to_double.exit):
            if place_to_double.ant:
                place_to_double.ant.double()
            place_to_double=place_to_double.exit
        super().action(gamestate)
        # END Problem 12
