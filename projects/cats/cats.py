...
def about(subject):
    ...
    def check_about(paragraph):
        good_paragraph=split(lower(remove_punctuation(paragraph)))
        for standard in subject:
            for word in good_paragraph:
                if word == standard:
                    return True
        return False
    return check_about
...
def accuracy(typed, source):
    ...
    if len_typed==0 and len_source==0:return 100.0
    if len_typed==0: return 0.0
    for i in range(len_typed):
        if i>=len_source:break
        if typed_words[i]==source_words[i]:sum+=1
    return sum/len_typed*100
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    most_likely_word=typed_word
    most_likely=limit+1
    for word in word_list:
        if word==typed_word:return word
        if diff_function(typed_word,word,limit)<most_likely and diff_function(typed_word,word,limit)<=limit:
            most_likely=diff_function(typed_word,word,limit)
...
def furry_fixes(typed, source, limit):
    ...
    if limit<0: return 0
    elif typed[0]!=source[0]: return 1+furry_fixes(typed[1:],source[1:],limit-1)
...
def minimum_mewtations(typed, source, limit):
    ...
    limit-=1
    return 1+min(minimum_mewtations(typed[1:],source,limit),minimum_mewtations(typed[1:],source[1:],limit),minimum_mewtations(typed,source[1:],limit))
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(source)):
        if len(typed)<=i or source[i]!=typed[i]:break
        progress+=1
    progress/=len(source)
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    def get_time(index,tpp):
        times=[]
        for i in range(len(tpp[index])-1):
            times+=[tpp[index][i+1]-tpp[index][i]]
        return times
    times=[[] for i in range(len(tpp))]
    for index in range(len(tpp)):
        times[index]=get_time(index,tpp)
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for iwords in word_indices:
        minn=0
        mint=times[0][iwords]
        for iplay in player_indices:
            if mint>times[iplay][iwords]:
                mint=times[iplay][iwords]
                minn=iplay
        result[minn]+=[words[iwords]]
    return result
    # END PROBLEM 10
