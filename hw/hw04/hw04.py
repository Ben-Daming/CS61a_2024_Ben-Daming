def shuffle(s):
    ...
    new_s=[]
    for i in range(even):
        new_s.append(s[odd+i])
        new_s.append(s[even+i])
    return new_s
...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    if (length(right(m))*total_mass(end(right(m))))==(length(left(m))*total_mass(end(left(m)))):
...
def max_path_sum(t):
    ...
    maxl=0
    for branch in branches(t):
        maxl=max(max_path_sum(branch),maxl)
    return maxl+label(t)
