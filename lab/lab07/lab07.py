    ...
    def time_to_retire(self, amount):
        ...
        def fast_exp(i,n):
            if not n:return 1
                ...
                return fast_exp(i,n-1)*i
            else:
                return fast_exp(i,n//2)**2
        year=0
        while fast_exp(self.interest+1,year)*self.balance<amount:
            year+=1
        return year
...
def duplicate_link(s, val):
    ...
    """
    "*** YOUR CODE HERE ***"
    if s is Link.empty:return
    elif s.first is val:
        s.first,s.rest=val,Link(val,s.rest)
