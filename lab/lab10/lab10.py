...
def floor_div(args):
    ...
    divident=calc_eval(args.first)
    if divident in OPERATORS:
        divident=calc_eval(args)
    dividers=args.rest
    while dividers is not nil:
        divident//=calc_eval(dividers.first)
        dividers = dividers.rest
    return divident
